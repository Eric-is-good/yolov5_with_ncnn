...
def check_wandb_config_file(data_config_file):
    wandb_config = '_wandb.'.join(data_config_file.rsplit('.', 1))  # updated archive.yaml path
...
class WandbLogger():
    ...
    def log_dataset_artifact(self, data_file, single_cls, project, overwrite_config=False):
        ...
        overwrite_config (boolean) -- overwrites the archive.yaml file if set to true otherwise creates a new
        ...
        if not log_val_only:
            path = (path.stem if overwrite_config else path.stem + '_wandb') + '.yaml'  # updated archive.yaml path
            path = ROOT / 'data' / path
